 ...
 * eip155:<digits> → itself, anything else → undefined (fail closed: an
 * unrecognised chain has no CAIP-2 here, ever). */
export function caip2Of(n: string): string | undefined {
  ...
  return EVM_CAIP2_RE.test(n) ? n : undefined;
}

/** The vocabulary name for a network string: a known alias stays the alias; a
 * known CAIP-2 id maps to its alias; an UNKNOWN eip155:* id returns itself
 * VERBATIM — never "base". Non-EVM strings yield "" (the caller falls through
 * to its other chain checks). */
